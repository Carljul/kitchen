<template>
    <div class="flex flex-col md:flex-row gap-[20px]">
        ...
        <div class="w-[400px]">
            ...
            <div class="sticky top-3 w-full bg-white p-5 rounded-md">
                ...
                <ul class="flex flex-col gap-[10px]">
                    ...
                    <li>
                        <router-link
                            :to="`${$route.params.type == this.$foodType ? '/products' : '/products/foods'}`"
                            :class="`${$route.params.type == this.$foodType ? 'bg-blue-500 text-white' : ''} flex p-3 border-[1px] border-slate-200 rounded-md shadow-lg mb-3 hover:bg-blue-500 hover:text-white justify-between items-center`"
                        >
                            <span>Foods</span>
                            <font-awesome-icon :icon="['fas', 'chevron-up']" :class="`${$route.params.type == this.$foodType ? 'block':'hidden'}`"/>
                            <font-awesome-icon :icon="['fas', 'chevron-down']"  :class="`${$route.params.type == this.$foodType ? 'hidden':'block'}`"/>
                        </router-link>
                        <ul :class="`${$route.params.type == this.$foodType ? 'block':'hidden'} flex flex-row md:flex-col gap-[10px] float-right w-full md:w-[90%] overflow-scroll md:overflow-auto`">
                    ...
                    <li>
                        <router-link
                            to="/products/bundles"
                            :class="`${$route.params.type == this.$bundleType ? 'bg-blue-500 text-white' : ''} flex p-3 border-[1px] border-slate-200 rounded-md shadow-lg mb-3 hover:bg-blue-500 hover:text-white justify-between items-center`"
                        >
                            <span>Bundles</span>
                            <font-awesome-icon :icon="['fas', 'chevron-up']" :class="`${$route.params.type == this.$bundleType ? 'block':'hidden'}`"/>
                            <font-awesome-icon :icon="['fas', 'chevron-down']"  :class="`${$route.params.type == this.$bundleType ? 'hidden':'block'}`"/>
                        </router-link>
                        
                        <ul :class="`${$route.params.type == this.$bundleType ? 'block':'hidden'} flex flex-row md:flex-col gap-[10px] float-right w-full md:w-[90%] overflow-scroll md:overflow-auto`">
        ...
        <div>
            <!-- Foods -->
            <div v-show="$route.params.type == this.$foodType || $route.params.type == undefined">
            ...
            <!-- Bundles -->
            <div v-show="$route.params.type == this.$bundleType || $route.params.type == undefined">
